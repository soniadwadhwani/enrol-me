...
interface ParentBasicInfoScreenProps {
  onBack: () => void;
  onContinue: (data: { fullName: string; mobile: string; email: string }) => void;
  currentStep: number;
  totalSteps: number;
}

export default function ParentBasicInfoScreen({ onBack, onContinue, currentStep, totalSteps }: ParentBasicInfoScreenProps) {
  const [fullName, setFullName] = useState('');
  const [mobile, setMobile] = useState('');
  const [email, setEmail] = useState('');

  const isValid = fullName.trim() && mobile.trim() && email.trim();

  const handleContinue = () => {
    if (isValid) {
      onContinue({ fullName, mobile, email });
    }
  };
